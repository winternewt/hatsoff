...
> **Status: research in progress.** Novelty is "yellow" — the
> matching↔zero-mode-percolation framework is established (Damle group, periodic
> lattices); the *aperiodic-monotile* substrate and the **p_c → 0** spanning
> result (finding #4) are the potentially-novel pieces, conditional on a
> non-proxy spanning analysis and a chiral-class literature check. See
...
| Dilution + Gallai–Edmonds | [`docs/PERCOLATION.md`](docs/PERCOLATION.md) | nullity ≈ deficiency; non-bipartite gap is small at L2 but **extensive and growing at L3**; trapped-weight fraction intensive (0→0.44); support spanning suggests **p_c → 0**. |
| Novelty / prior art | [`docs/NOVELTY.md`](docs/NOVELTY.md) | Per-finding (a/b/c) verdict + closest references + competitor (Damle group). |
...
docs/        NULLITY, DISORDER, PERCOLATION, NOVELTY, PLAN, THEHAT, deepresearch_prompt
...
(arXiv identifiers are as located during this project; the very recent
preprints in 5 above were not all independently verified — confirm before use.)
